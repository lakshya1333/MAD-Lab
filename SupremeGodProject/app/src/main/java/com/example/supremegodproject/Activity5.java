...
public class Activity5 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnInsert.setOnClickListener(v -> {
            boolean isInserted = dbHelper.insertData(etName.getText().toString(), etValue1.getText().toString(), "Act5", "Act5");
            Toast.makeText(this, isInserted ? "Inserted" : "Failed", Toast.LENGTH_SHORT).show();
        });

        // Update
        btnUpdate.setOnClickListener(v -> {
            int rows = dbHelper.updateData(etID.getText().toString(), etName.getText().toString(), etValue1.getText().toString(), "U5", "U5");
        ...
        btnNext.setOnClickListener(v -> {
            // Loop back to Activity 1
            startActivity(new Intent(this, Activity1.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
        });
